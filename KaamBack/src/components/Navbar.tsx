...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full flex items-center lg:justify-between px-6 lg:px-24 lg:py-6 py-4 transition-all duration-300 ${navBackground ? 'bg-white border-b-[3px] shadow-xl' : 'bg-[#041893] border-none'} z-50`}>
      <div className="lg:hidden mr-6">
        <button onClick={() => setIsOpen(!isOpen)}>
          {isOpen ? <FaTimes size={30} className={`${navBackground ? 'text-[#041893]' : 'text-[#fafcfd]'}`} /> : <FaBars size={30} className={`${navBackground ? 'text-[#041893]' : 'text-[#fafcfd]'}`} />}
        </button>
      </div>
